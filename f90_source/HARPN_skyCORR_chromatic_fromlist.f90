...
character (len=nch_file) :: order_list_file, file_tbl_A, &
  previous_wave_A, previous_wave_B, previous_lamp_A, previous_lamp_B

character (len=1) :: xi
logical :: save_skyr, save_copy
...
if (trim(archive_dir) .eq. trim(output_dir)) then
    save_copy = .False.
else
    save_copy = .True.
end if
...
do while (read_iostat.eq.0)
    ...
    if (date_ccf.eq.1) then
        ...
        file_tbl_A = trim(archive_dir) // '/' // trim(file_date)// '/' // trim(file_rad_obs) // '_ccf_'//trim(file_mask)//'_A.tbl'
    else
        ...
        file_tbl_A = trim(archive_dir) // '/' // trim(file_rad_obs) // '_ccf_'//trim(file_mask)//'_A.tbl'
    end if
    ...
    call fits_check(file_2d_A,sts_out)
    if (sts_out.eq.0) then
        file_2d_A = previous_wave_A
        file_2d_B = previous_wave_B
        write(*,*) 'Using WAVE files from previous observation in the list'
    else
        previous_wave_A = file_2d_A
        previous_wave_B = file_2d_B
    end if
    ...
    !!! Copying CCF and TBL files of fibre A
    if (save_copy) then
      if (date_output.eq.1) then
          ...
          call system('cp ' // trim(file_tbl_A) // ' ' //  trim(output_dir) // '/' // trim(file_date) // '/')
      else
          call system('mkdir -p ' // trim(output_dir))
          call system('cp ' // trim(file_ccf_A) // ' ' //  trim(output_dir) // '/' )
          call system('cp ' // trim(file_tbl_A) // ' ' //  trim(output_dir) // '/' )
      end if
      write(*,*) 'File copied: ',  trim(file_ccf_A)
      write(*,*) 'File copied: ',  trim(file_tbl_A)
    end if

    save_skyr = .True.

    call fits_check(file_ccf_B,sts_out)
    write(*,*) 'Fiber B on sky (False=0): ', sts_out

    if (sts_out.eq.0) save_skyr=.False.


    if (save_skyr.eqv. .True.) then
        ...
        call fits_check(file_2d_A,sts_out)
        if (sts_out.eq.0) then
            file_2d_A = previous_lamp_A
            file_2d_B = previous_lamp_B
            write(*,*) 'Using LAMP files from previous observation in the list'
        else
            previous_lamp_A = file_2d_A
            previous_lamp_B = file_2d_B
        end if
        ...
        write(*,*) 'File saved:  ',  trim(file_ccf_O)
    ...
    CCF_A_chromatic = 0._PR
    CCF_O_chromatic = 0._PR
    chromatic_orders = 0
    ...
    do ic=1,chromatic_n_ranges
        ...
        close(lun_out)
        write(*,*) 'File saved:  ',  trim(order_list_file)
    ...
    if (date_output.eq.1) then
        ...
        file_ccf_O =  trim(output_dir) // '/' // trim(file_rad_obs) // &
          '_ccf_'//trim(file_mask)//'_A_chromatic.fits'
    end if
