...
plotting = False 	#set True/False to toggle on/off plotting capabilities"
...
def read_obslist(listname, filetype, RVext):
	...
	if(filetype in ["HARPS", "HARPS-N", "SOAP"]):
		#then pyfits will be used to read the files
...
def read_header(filename):
	""" reads data from a file , ignoring lines NOT starting with the character
	"#". Returns a list with the blank-separated elements of each line """

	f = open(filename, "r")
	with open(filename, "r") as f:
		list_data = [line.split() for line in f.readlines() if line.startswith('#')]
...
def create_header(output, indTag, slope_params, chi2, std_res, rPearson_data, dev_data, prob_data):
	...
	with open(output, "w") as f:
		...
		f.write("# The fitted slope was (m,b) (%f, %f) with a rchi2 of %f and a res. st.d. of %f\n" % (slope_params[0], slope_params[1], chi2, std_res))
		f.write("# The RV and indicator dataset have an associated Pearson's correlation coefficient of %f, being at %f sigma from an uncorrelated shuffled distribution.\n" \
			% (rPearson_data, dev_data))
		f.write("# The single-sided Gaussian p-value of such an event is %f %%\n" % (prob_data*100.0))
...
def wrapRes(output_ASCII_list, BJDs, resultsdir):
	...
	#reading the headers of the data to recover the stored p-values
	p_values = np.array([float(read_header(file_ind)[3][-2]) for file_ind in output_ASCII_list])
	p_values_desc_indx = np.argsort(p_values)[::-1]	#index for sorted values, in descending order  
	nr_tests = len(p_values)
	q_values = []
      
	# Sidak Correction
	sidak_values = [ (1.0-(1.0-p_v/100.0)**nr_tests)*100.0 for p_v in p_values]
	
	for i in range(nr_tests):
		
		# selecting the j for the formula of  Benjamini and Hochberg (1995)
		j = np.where(p_values_desc_indx==i)[0][0] # selecting the index when ordering in a step-down way
		q_values.append(np.min([p_values[i]*nr_tests/(nr_tests-i), 100.0]))
	
	FileNameMHT = 'WrapUpMHT_%(time)s' %{'time':time_stamp}
	output_WrapUpMHT = '%(dir)s%(name)s.txt'%{'dir':resultsdir,'name':FileNameMHT}
	
	with open(output_WrapUpMHT, "w") as f:
		f.write("# Wrap-up of Multi-hypothesis Testing correction.\n")
		f.write("#\t Indicator \t p-value \t sidak-values \t q-value\n")
	
		data_to_file = zip(indTag_list, p_values, sidak_values, q_values)

		[f.write("\t"+line[0]+"\t\t"+str(line[1])+"\t"+str(line[2])+"\t"+str(line[3])+"\n") for line in data_to_file]
	
	print '\n'
	print_line('WrapUp', 'Multi-Hipothesis Testing correction for the results stored in ASCII format in %s' %output_WrapUpMHT)
...
def read_data(fitsfilename):
	...
	#Read the FITS file and return an array containing the data. """
  	...
  	filep = pyfits.open(fitsfilename);
  	return filep[0].data;
...
def read_obsFITS(obs_file, filetype):
	...
	if(filetype=="HARPS"):
		...
		sumCCF = data_st
		crval1 = -1*read_kw(obs_file, "WIDTH0")
		cdelt1 = read_kw(obs_file, "STEP")
