...
def Vasy(RV, CCF, Gauss_params, plot = 0):
	...
	center_G = Gauss_params[1]
	#update: RV table and Gaussian center is centered at zero  
	RV_cen = RV - np.average(RV)
	Gauss_params_cen = [Gauss_params[0], Gauss_params[1] - np.average(RV), Gauss_params[2], Gauss_params[3]]
	dummy1, dummy2 = 0, 0	# created as dummys to obtain the same number of outputs
	
	depth, lambda_b, lambda_r, v0 = CCF_interpolate(RV_cen, CCF, Gauss_params_cen)
