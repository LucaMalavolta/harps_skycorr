module instrument_harpn
  ...
  character (len=*), parameter :: archive_raw = archive_raw_harpn
  character (len=*), parameter :: archive_red = archive_red_harpn

  ! fits images informations (HARPS)
  integer, parameter :: n_order = 69, n_pix = 4096

  integer, parameter :: starting_order = 1
...
!!! ADC variables -> out-dated!
  ...
  integer ,parameter :: no_ref = 51 !! This is the order where the wl=5500 is located
  ...
  character (len=*), parameter :: sun_continuum = trim(tables_path) // 'HARPN_Ganymede_norm_poly.fits'
...
contains
  subroutine singleheader_RVC(unit,rv_fin,sts)
    ...
    sts = 0
    rv_fin = 0._PR
    call FTGKYD(unit,'HIERARCH TNG DRS CCF RVC',rv_fin,comment,sts);  sts = 0
    return
  ...
  subroutine singleheader_BERV(unit,berv,sts)
    ...
    sts = 0
    berv = 0._PR
    call FTGKYD(unit,'HIERARCH TNG DRS BERV',berv,comment,sts);  sts = 0
    return
  ...
  subroutine singleheader_BLAZE(unit,file_blaze,sts)
    ...
    sts = 0
    file_blaze = ''
    call FTGKLS(unit,'HIERARCH TNG DRS BLAZE FILE',file_blaze,comment,sts);  sts = 0
    return
  ...
  subroutine singleheader_CCD_GAIN(unit,ccd_gain,sts)
    ...
    call FTGKYE(unit,'HIERARCH TNG DRS SPE EXT SN'//trim(ord_chr),snr_key,comment,sts);  sts = 0
    return
